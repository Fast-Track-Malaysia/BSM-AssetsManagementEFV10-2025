...
namespace AssetsManagementEF.Module.BusinessObjects
{
    ...
    [XafDisplayName("Work Order Equipment")]
    //[DefaultListViewOptions(MasterDetailMode.ListViewOnly, false, NewItemRowPosition.None)]
    // Specify more UI options using a declarative approach (https://documentation.devexpress.com/#eXpressAppFramework/CustomDocument112701).
    [RuleCriteria("WorkOrderEquipmentsDeleteRule", DefaultContexts.Delete, "1=0", "Cannot Delete.")]
    //[RuleCriteria("WorkOrderEquipmentsEqSaveRule", DefaultContexts.Save, "ValidDate", "Date From and Date To is not valid.")]
    public class WorkOrderEquipments : IXafEntityObject, IObjectSpaceLink, INotifyPropertyChanged
    {
        public WorkOrderEquipments()
        {
            // In the constructor, initialize collection properties, e.g.: 
            this.Detail = new List<WorkOrderEqComponents>();
            this.DetailOp = new List<WorkOrderEquipmentOps>();
        ...
        public DateTime? EqDownTime
        {
            get
            {
                ...
                if (DetailOp.Where(p => p.WorkOrderOpType.IsDown).Count() <= 0)
                    return null;
                return DetailOp.Where(p => p.WorkOrderOpType.IsDown).Min(p => p.EqDownTime) ;
            }
        }
        [ModelDefault("DisplayFormat", "{0:f}")]
        [System.ComponentModel.DataAnnotations.Schema.NotMapped]
        ...
        public DateTime? EqUpTime
        {
            get
            {
                ...
                if (DetailOp.Where(p => p.WorkOrderOpType.IsUp).Count() <= 0)
                    return null;
                return DetailOp.Where(p => p.WorkOrderOpType.IsUp).Max(p => p.EqUpTime);
            }
        }
        ...
        public virtual IList<WorkOrderEqComponents> Detail { get; set; }

        [XafDisplayName("Equipment Operation")]
        public virtual IList<WorkOrderEquipmentOps> DetailOp { get; set; }
