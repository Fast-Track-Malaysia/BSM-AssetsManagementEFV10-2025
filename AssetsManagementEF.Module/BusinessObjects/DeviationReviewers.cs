...
namespace AssetsManagementEF.Module.BusinessObjects
{
    ...
    //[Appearance("HideNew", AppearanceItemType = "Action", TargetItems = "Delete", Context = "Any", Visibility = DevExpress.ExpressApp.Editors.ViewItemVisibility.Hide)]
    ...
    public class DeviationReviewers : IXafEntityObject, IObjectSpaceLink, INotifyPropertyChanged
    {
        public DeviationReviewers()
        {
        ...
        private SystemUsers _Reviewer;
        [XafDisplayName("Reviewer"), ToolTip("Enter Text")]
        //[ModelDefault("EditMask", "(000)-00"), VisibleInListView(false)]
        [Index(2), VisibleInListView(true), VisibleInDetailView(true), VisibleInLookupListView(true)]
        [RuleRequiredField(DefaultContexts.Save)]
        [ImmediatePostData(true)]
        public virtual SystemUsers Reviewer
        {
            ...
            set
            {
                if (_Reviewer != value)
                {
                    ...
                    if (_Reviewer != null)
                    {
                        Position = objectSpace.FindObject<Positions>(CriteriaOperator.Parse("CurrentUser.ID=?", _Reviewer.ID));
        ...
        {
            get { return _PositionRole; }
            set
            {
                if (_PositionRole != value)
                {
                    _PositionRole = value;
                    OnPropertyChanged("PositionRole");
                }
            }
        }

        private bool _KeyReviewer;
        [XafDisplayName("Key")]
        //[ModelDefault("EditMask", "(000)-00"), VisibleInListView(false)]
        [Index(5), VisibleInListView(true), VisibleInDetailView(true), VisibleInLookupListView(true)]
        public bool KeyReviewer
        {
            get { return _KeyReviewer; }
            set
            {
                if (_KeyReviewer != value)
                {
                    _KeyReviewer = value;
                    OnPropertyChanged("KeyReviewer");
                }
            }
        }
        private bool _ActReviewer;
        [XafDisplayName("Ack")]
        //[ModelDefault("EditMask", "(000)-00"), VisibleInListView(false)]
        [Index(6), VisibleInListView(true), VisibleInDetailView(true), VisibleInLookupListView(true)]
        [Appearance("ActReviewer", Enabled = false)]
        public bool ActReviewer
        {
            get { return _ActReviewer; }
            set
            {
                if (_ActReviewer != value)
                {
                    _ActReviewer = value;
                    OnPropertyChanged("ActReviewer");
                }
            }
        }

        private DateTime? _ReviewDate;
        [XafDisplayName("Review Date")]
        //[ModelDefault("EditMask", "(000)-00"), VisibleInListView(false)]
        [Index(10), VisibleInListView(true), VisibleInDetailView(true), VisibleInLookupListView(true)]
        [Appearance("ReviewDate", Enabled = false)]
        ...
        void IXafEntityObject.OnCreated()
        {
            ...
            //Reviewer = objectSpace.GetObjectByKey<SystemUsers>(SecuritySystem.CurrentUserId);
