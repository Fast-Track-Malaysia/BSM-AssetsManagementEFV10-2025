...
namespace AssetsManagementEF.Module.BusinessObjects
{
    ...
    [DefaultProperty("EquipmentComponent")]
    //[DefaultListViewOptions(MasterDetailMode.ListViewOnly, false, NewItemRowPosition.None)]
    // Specify more UI options using a declarative approach (https://documentation.devexpress.com/#eXpressAppFramework/CustomDocument112701).
    [RuleCriteria("WorkOrderEqComponentsDeleteRule", DefaultContexts.Delete, "1=0", "Cannot Delete.")]
    //[RuleCriteria("WorkOrderEqComponentsEqSaveRule", DefaultContexts.Save, "ValidDate", "Date From and Date To is not valid.")]
    public class WorkOrderEqComponents : IXafEntityObject, IObjectSpaceLink, INotifyPropertyChanged
    {
        public WorkOrderEqComponents()
        {
            // In the constructor, initialize collection properties, e.g.: 
            this.DetailOp = new List<WorkOrderEqComponentOps>();
        }
        ...
        [Appearance("_Equipment", Enabled = false, Criteria = "not IsNew")]
        ...
        EquipmentComponents _EquipmentComponent;
        ...
        public virtual EquipmentComponents EquipmentComponent
        {
            get { return _EquipmentComponent; }
            set
            {
                if (_EquipmentComponent != value)
                {
                    _EquipmentComponent = value;
                    OnPropertyChanged("EquipmentComponent");
                }
            }
        }
        ...
        {
            get { return EquipmentComponent == null ? "" : EquipmentComponent.BoName; }
        }
        ...
        public int OpDoneCount
        {
            get
            {
                return DetailOp.Where(p => p.IsDone = true)
                    .Count();
            }
        }
        ...
        [System.ComponentModel.DataAnnotations.Schema.NotMapped]
        [Browsable(false)]
        public DateTime? CpDownTime
        {
            get
            {
                if (DetailOp == null || DetailOp.Count == 0)
                    return null;
                if (DetailOp.Where(p => p.WorkOrderOpType.IsDown).Count() <= 0)
                    return null;
                return DetailOp.Where(p => p.WorkOrderOpType.IsDown).Min(p => p.CpDownTime);
            }
        }
        [ModelDefault("DisplayFormat", "{0:f}")]
        [System.ComponentModel.DataAnnotations.Schema.NotMapped]
        [Browsable(false)]
        public DateTime? CpUpTime
        {
            get
            {
                if (DetailOp == null || DetailOp.Count == 0)
                    return null;
                if (DetailOp.Where(p => p.WorkOrderOpType.IsUp).Count() <= 0)
                    return null;
                return DetailOp.Where(p => p.WorkOrderOpType.IsUp).Max(p => p.CpUpTime);
            }
        }
        ...
        public long ManHours
        {
            get
            {
                return DetailOp.Sum(p => p.ManHours);
            }
        }
        ...
        {
            get { return WorkOrder == null ? "" : WorkOrder.DocStatus; }
        }

        // Collection property:
        [XafDisplayName("Component Operation")]
        public virtual IList<WorkOrderEqComponentOps> DetailOp { get; set; }
